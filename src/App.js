...
const Engine = {
    ...
    decode: (b64) => {
        try {
            // الخطوة 1: تنظيف المدخل من markdown ومسافات زائدة
            let clean = b64.trim();
            const codeBlockMatch = clean.match(/```(?:[a-zA-Z]*\n?)?([\s\S]*?)```/);
            if (codeBlockMatch) clean = codeBlockMatch[1].trim();
            clean = clean.replace(/`/g, '').trim();

            // الخطوة 2: فك تشفير Base64 (مع fallback إذا كان JSON مباشراً)
            let decoded;
            try { decoded = atob(clean); } catch { decoded = clean; }

            // الخطوة 3: تنظيف decoded من markdown داخلي
            decoded = decoded.trim();
            const innerBlock = decoded.match(/```(?:[a-zA-Z]*\n?)?([\s\S]*?)```/);
            if (innerBlock) decoded = innerBlock[1].trim();

            // الخطوة 4: parse JSON بمرونة
            let parsed;
            try {
                parsed = JSON.parse(decoded);
            } catch {
                const arrMatch = decoded.match(/\[[\s\S]*\]/);
                const objMatch = decoded.match(/\{[\s\S]*\}/);
                if (arrMatch) { try { parsed = JSON.parse(arrMatch[0]); } catch { return null; } }
                else if (objMatch) { try { parsed = JSON.parse(objMatch[0]); } catch { return null; } }
                else { return null; }
            }

            // الخطوة 5: توحيد الهيكل إلى مصفوفة dims
            let dims = [];

            if (Array.isArray(parsed)) {
                if (parsed.length > 0 && typeof parsed[0] === 'object') {
                    dims = parsed;
                } else if (parsed.length === 30 && typeof parsed[0] === 'number') {
                    // 30 رقم مسطّح: كل 3 = بُعد
                    for (let i = 0; i < 30; i += 3) {
                        dims.push({ C: parsed[i], S: parsed[i+1], V: parsed[i+2] });
                    }
                } else if (parsed.length >= 10 && typeof parsed[0] === 'number') {
                    // 10 أرقام composite مباشرة
                    const result = parsed.slice(0, 10).map(v => Engine.normalizeComposite(Number(v)));
                    while (result.length < 10) result.push(0);
                    return result;
                }
            } else if (typeof parsed === 'object' && parsed !== null) {
                // object: ابحث عن array داخلية أولاً
                const arrayKey = Object.keys(parsed).find(k => Array.isArray(parsed[k]));
                if (arrayKey) {
                    const arr = parsed[arrayKey];
                    if (arr.length > 0 && typeof arr[0] === 'object') dims = arr;
                } else {
                    // object مرقّم أو مسمّى: خذ القيم
                    const vals = Object.values(parsed);
                    if (vals.length > 0 && typeof vals[0] === 'object') dims = vals;
                }
            }

            if (dims.length === 0) return null;

            // الخطوة 6: استخراج C, S, V بأسماء مرنة
            const result = dims.slice(0, 10).map(dim => {
                const C = Number(dim.C ?? dim.core ?? dim.Core ?? dim.CORE ?? 7.5);
                const S = Number(dim.S ?? dim.shadow ?? dim.Shadow ?? dim.SHADOW ?? 7.5);
                let V   = Number(dim.V ?? dim.velocity ?? dim.Velocity ?? dim.VELOCITY ?? 0);

                // إذا كان V خارج [-1,1] افترض أنه مقياس [0,15]
...
export default function App() {
    ...
    const handleLogin = async () => {
        ...
        setAuthMode(null);
        // تحقق من وجود بصمة محفوظة
        const { data: profile } = await supabase
            .from('profiles')
            .select('vector_data')
            .eq('id', data.user.id)
            .single();
        if (profile?.vector_data && Array.isArray(profile.vector_data) && profile.vector_data.length >= 10) {
            const { data: users } = await supabase
                .from('profiles')
                .select('username, vector_data')
                .not('vector_data', 'is', null);
            const scored = (users || [])
                .filter(u => Array.isArray(u.vector_data) && u.vector_data.length >= 10)
                .filter(u => u.username !== username.trim())
                .map(u => {
                    const rawSim = Engine.calculate(profile.vector_data, u.vector_data);
    ...
    const handleMatch = async () => {
        ...
        try {
            ...
            if (!userVector || userVector.length < 10) {
                alert("خطأ في الصيغة: تأكد أن المدخل Base64 صحيح ويحتوي على 10 أبعاد على الأقل");
                setLoading(false);
                return;
            }
            ...
            // ✅ حساب التشابه الكوساني لكل مستخدم (مع استبعاد المستخدم الحالي)
            const scored = (users || [])
                .filter(u => Array.isArray(u.vector_data) && u.vector_data.length >= 10)
                        ...
                        <div className="w-[90%] h-[50vh] border border-amber-300/20 rounded-[2.5rem] bg-black/40 overflow-y-auto p-6">
                            <table className="w-full border-collapse" dir={lang === 'AR' ? "rtl" : "ltr"}>
                                <tbody>
                                    ...
                                    )) : <tr><td colSpan="2" className="text-center py-20 text-amber-300/40 text-4xl italic">{lang === 'AR' ? 'جارٍ البحث عن التوائم...' : lang === 'RU' ? 'Поиск совпадений...' : 'Finding matches...'}</td></tr>}
                                </tbody>
                            </table>
                        </div>
                        <button onClick={() => { setUserInput(''); setView('onboarding'); }} className="mt-8 flex flex-col items-center gap-2 py-6 px-16 border-2 border-amber-300 rounded-full text-amber-300 active:scale-95">
                            <Scan size={42} />
